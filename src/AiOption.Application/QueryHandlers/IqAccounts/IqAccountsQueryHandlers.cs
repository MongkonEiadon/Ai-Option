...
{
    class IqAccountsQueryHandlers : IQueryHandler<QueryNewValidTokenForUser, string>
    {
        ...
        public async Task<string> ExecuteQueryAsync(QueryNewValidTokenForUser query, CancellationToken cancellationToken)
        {
            var result =  await _apiWrapper.LoginToIqOptionAsync(query.User.Value, query.Password.Value);

            if (!result.Item1)
            {
                throw DomainError.With($"Get credential for '{query.User}' failed with {result.Item2}");
            }
