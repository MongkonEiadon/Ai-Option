...
namespace iqoption.data.Commands {
    public class StoreSsidCommandHandler {
        ...
        public async Task<string> HandleRequestAsync(StoreSsidCommand request,
            CancellationToken ct = default(CancellationToken)) {
            var account =
                await _accountDto.FirstOrDefaultAsync(x =>
                    x.IqOptionUserName.ToUpper() == request.EmailAddress.ToUpper());
